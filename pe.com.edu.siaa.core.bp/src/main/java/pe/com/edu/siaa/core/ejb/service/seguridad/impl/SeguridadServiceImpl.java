...
public class SeguridadServiceImpl implements SeguridadServiceLocal{
	...
	public UsuarioDTO integracionUsuario(UsuarioDTO usuario,AccionType accionType) throws Exception {
		...
		if (!AccionType.CREAR.equals(accionType)) {
			System.out.println("CodigoEx: " +usuario.getCodigoExterno());
			usuarioEntity = this.usuarioDaoImpl.obtenerUsuarioByCodigoExterno(usuario.getCodigoExterno());
			if(StringUtils.isNullOrEmpty(usuarioEntity)){
				usuario.setIdUsuario(this.usuarioDaoImpl.generarIdUsuario());
				resultadoEntity = TransferDataObjectUtil.transferObjetoEntity(usuario, Usuario.class,"tipoUsuario@PK@");
				this.usuarioDaoImpl.save(resultadoEntity);	
			}
		...
		switch (accionType) {
			...
			case MODIFICAR:
				if(!StringUtils.isNullOrEmpty(usuarioEntity)){
					...
					this.usuarioDaoImpl.save(resultadoEntity);	
					//Inicio creando usuario
					UsuarioEntidad usuarioEntidad2 = new UsuarioEntidad();
					usuarioEntidad2.setIdUsuarioEntidad(this.usuarioEntidadDaoImpl.generarIdUsuarioEntidad());
					usuarioEntidad2.setEntidad(new Entidad());
					usuarioEntidad2.getEntidad().setIdEntidad(usuario.getIdEntidadSelect());
					usuarioEntidad2.setUsuario(resultadoEntity);
					usuarioEntidad2.setEstado(EstadoGeneralState.ACTIVO.getKey());
					this.usuarioEntidadDaoImpl.save(usuarioEntidad2);
					//Fin creando usuario
					resultado = usuario;
					break;
				}*/
	...
	@Override
	public UsuarioDTO findByUsuario(String userName) throws Exception {
		UsuarioDTO resultado = new UsuarioDTO();
		Usuario usuario = usuarioDaoImpl.findUsuario(userName);
		resultado = TransferDataObjectUtil.transferObjetoEntityDTO(usuario,UsuarioDTO.class);
		usuario = null;
		return resultado;
	}
