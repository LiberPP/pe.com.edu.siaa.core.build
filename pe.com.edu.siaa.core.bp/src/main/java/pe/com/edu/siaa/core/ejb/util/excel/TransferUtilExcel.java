...
public class TransferUtilExcel {
     ...
     * @param sheet el sheet
     */
    public static void copySheetsXLSX(SXSSFSheet newSheet, XSSFSheet sheet){   
     ...
     */
    public static void copySheetsXLSX(SXSSFSheet newSheet, XSSFSheet sheet, boolean copyStyle){   
        int maxColumnNum = 0;   
    ...
    public static void copyRowXLSX(XSSFSheet srcSheet, SXSSFSheet destSheet, XSSFRow srcRow, SXSSFRow destRow, Map<Integer, XSSFCellStyle> styleMap) {   
        // manage a list of merged zone in order to not insert two times a merged zone
      Set<CellRangeAddressWrapper> mergedRegions = new TreeSet<CellRangeAddressWrapper>();   
        destRow.setHeight(srcRow.getHeight());   
        // reckoning delta rows
    ...
    public static void copyCell(XSSFCell oldCell, SXSSFCell newCell, Map<Integer, XSSFCellStyle> styleMap) {   
        if(styleMap != null) {   
            if(oldCell.getSheet().getWorkbook().getCTWorkbook() == newCell.getSheet().getWorkbook()){   
                newCell.setCellStyle(oldCell.getCellStyle());   
            } else{   
                int stHashCode = oldCell.getCellStyle().hashCode();   
    ...
    public static CellRangeAddress getMergedRegionXLSX(XSSFSheet sheet, int rowNum, short cellNum) {   
    	for (int i = 0; i < sheet.getNumMergedRegions(); i++) { 
            CellRangeAddress merged = sheet.getMergedRegion(i);   
            if (merged.isInRange(rowNum, cellNum)) {   
                return merged;   
