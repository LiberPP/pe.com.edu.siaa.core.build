...
import pe.com.edu.siaa.core.model.type.CarpetaType;
...
public class AdministradorReportes implements Serializable {
	...
	/** La log. */
	private static Logger log = Logger.getLogger(AdministradorReportes.class);
	...
	public static final String RUTA_REPORTE_GENERADO =   ConstanteConfigUtil.SEPARADOR_FILE + CarpetaType.REPORTE_GENERADO.getKey();
	...
	private static JasperPrintVO generarJasperPrint(ParametroReporteVO parametroReporteVO) throws Exception {
		...
		File source = new File(parametroReporteVO.getRutaReportesAbsoluto() + parametroReporteVO.getJasper().getKey());
		

		for (int i = 0; i < parametroReporteVO.getSubreportes().length; i++) {
			// Carga cada subreporte
			JasperReport subreporte = (JasperReport) JRLoader.loadObject(new File(source.getParent(), parametroReporteVO.getSubreportes()[i]));
		...
		if (parametroReporteVO.isOnline()) {
			SwapVirtualizadorCacheUtil.getInstance().put(parametroReporteVO.getUserName(), parametroReporteVO.getParametros().get("codigoSolicitud") + "", virtualizer);
		}
		...
		jasperPrint = JasperFillManager.fillReport(jasperReport, parametroReporteVO.getParametros(), parametroReporteVO.getCn());
	...
	private static JRSwapFile generarRutaSwap (ParametroReporteVO parametroReporteVO) {
		String rutaSession = ConstanteConfigUtil.RUTA_RECURSOS_SWAP_FILE + ConstanteConfigUtil.generarRuta(parametroReporteVO.getUserName()) ;
		if (parametroReporteVO.isOnline()) {
			rutaSession = ConstanteConfigUtil.RUTA_SESSION_TEMP + ConstanteConfigUtil.generarRuta(parametroReporteVO.getUserName()) ;
		}
		File file = new File(rutaSession + parametroReporteVO.getParametros().get("codigoSolicitud"));
		if (!file.exists()) {
			file.mkdirs();
		}
		JRSwapFile swapFile = new JRSwapFile(rutaSession  + parametroReporteVO.getParametros().get("codigoSolicitud") , 100, 100);
		return swapFile;
	}
	...
	private static JasperPrintVO generarJasperPrintsinConexion(ParametroReporteVO parametroReporteVO) {
		...
		try {
			...
			if(jasperPrint.getPages().size()<=500){
				for (Iterator<JRPrintPage> iterator = jasperPrint.getPages().iterator(); iterator.hasNext();) { //Remueve páginas blancas si existieran
	...
	public static Map<String, Object> generarReporteArrayPdf(ParametroReporteVO parametroReporteVO) throws Exception {
		...
		try {
			...
			if (parametroReporteVO.isBigMemory()) {
				...
				ArchivoUtilidades.copyArchivoBigMemory(nombreArchivo, nombreArchivo, parametroReporteVO.getUserName(), false);
			}
			parametroReporteVO = null;
			jaspertPrintVirtualizadorVO = null; //LIBERAR MEMORIA
		} catch (JRException e) {
			log.error(e);
		}
	...
	private static void generarReporteVirtualizerPdf(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
	...
	public static void generarReporteVirtualizerXls(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerXlsx(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerDocx(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerHtml(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
		generarReporteArrayHtml(jaspertPrintVirtualizadorVO.getJasperPrint(),(String)parametroReporteVO.getParametros().get(ConstanteGenerarReporteUtil.CONTEXT_URL),nombreArchivoSalida);
		jaspertPrintVirtualizadorVO.getVirtualizer().cleanup();  //LIBERAR DISCO
		limpiarRutaSwap(parametroReporteVO);
		jaspertPrintVirtualizadorVO = null;
	}
	public static void generarReporteArrayHtml(JasperPrint jasperPrint,String context,String ruta) {
		try {
			...
			output.setImageHandler(new WebHtmlResourceHandler("" + context + "/resourcesweb/reporte/images/logo_mapfre_general.jpg"));
	...
	public static void generarReporteVirtualizerRtf(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerOdt(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerOds(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteVirtualizerPptx(ParametroReporteVO parametroReporteVO,JasperPrintVO jaspertPrintVirtualizadorVO) throws Exception {
		String nombreArchivoSalida = generarRutaReporte(parametroReporteVO);
	...
	public static void generarReporteArrayXlsx2(JasperPrint jasperPrint,String ruta) {
		...
		try {
			...
			//jrXlsExporter.setParameter(JRXlsExporterParameter.SHEET_NAMES, new String[] {"mI REPORTE"});
			jrXlsExporter.exportReport();
		} catch (JRException e) {
			log.error(e);
		}
	...
	private  static void limpiarRutaSwap(ParametroReporteVO parametroReporteVO) throws Exception {
		...
		ArchivoUtilidades.limpiarArchivoAllDirectory(rutaSession + parametroReporteVO.getParametros().get("codigoSolicitud"));
	}
	public  static String generarRutaReporte(ParametroReporteVO parametroReporteVO) {
		...
		nombreArchivoSalida = nombreArchivoSalida + ConstanteConfigUtil.SEPARADOR_FILE + parametroReporteVO.getUserName()  ;
		File file = new File(nombreArchivoSalida );
		if (!file.exists()) {
			file.mkdirs();
		}
		nombreArchivoSalida = nombreArchivoSalida   + ConstanteConfigUtil.SEPARADOR_FILE + parametroReporteVO.getFileName() + "." + parametroReporteVO.getFormato();
