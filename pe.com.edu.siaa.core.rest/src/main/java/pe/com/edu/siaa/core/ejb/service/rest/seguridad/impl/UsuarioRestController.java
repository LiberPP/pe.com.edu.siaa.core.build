...
public class UsuarioRestController extends GenericServiceRestImpl {
	...
	@GET
	@Path("/findUsuario/{userName}")
	public ResultadoRestVO<UsuarioDTO> findUsuario(@PathParam("userName")String userName) throws Exception {
		ResultadoRestVO<UsuarioDTO> resultado = new ResultadoRestVO<UsuarioDTO>();
		 try {
   		resultado.setObjetoResultado(seguridadServiceLocal.findByUsuario(userName));
		} catch (Exception e) {
			parsearResultadoError(e, resultado);
		}
		return resultado;
