package pe.com.edu.siaa.core.ejb.dao.impl;
...
public class ReporteDaoImpl implements ReporteDaoLocal {
	...
	public Map<String,Object> generarReporteArrayPdf(ParametroReporteVO parametroReporteVO) throws Exception {
		String nombreJNDIConexion = parametroReporteVO.getNombreJNDIConexion();
		if (!StringUtils.isNotNullOrBlank(nombreJNDIConexion)) {
			nombreJNDIConexion = "java:jboss/datasources/siaaDS";
		}
		Connection conexion = getConexionDS(nombreJNDIConexion); //TODO 
	...
	private ParametroReporteVO completarParametros(ParametroReporteVO parametroReporteVO) {
		 ...
		 if (parametroReporteVO.getJasper() == null) {
			 ...
			 rutaReportes = ConstanteConfigUtil.generarRuta(ConstanteConfigUtil.RUTA_REPORTE, parametroReporteVO.getJasper().getCarpeta() , parametroReporteVO.getRutaReporteType().getKey());
		 }
		 ...
		 //parametroReporteVO.getParametros().put("REPORT_LOCALE", ES);//PARA FORMATO DEL DECIMAL #,##.00
		 parametroReporteVO.getParametros().put(JRParameter.REPORT_LOCALE, ES);//PARA FORMATO DEL DECIMAL #,##.00
